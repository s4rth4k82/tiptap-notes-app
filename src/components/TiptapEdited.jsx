...
const TiptapEpited = ({ notes, allNotes, note, editing, setEditing }) => {
  ...
  const saveEditedContent = async (noteId) => {
    try {
      const html = editor.getHTML();
      ...
      {editing ? (
        <div className="w-[70%] m-8 mx-auto">
          ...
          <button
            ...
            onClick={() => saveEditedContent(note._id)}
          >
            SAVE
          </button>
        </div>
